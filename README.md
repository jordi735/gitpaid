...
| `src/routes/github.ts`    | Three routes — `/user`, `/orgs`, `/commits` — all gated by `requireToken` middleware.    |
| `src/lib/github-client.ts` | Thin wrapper around GitHub REST v3 with pagination, friendly error mapping, and a 1s courtesy delay between search pages. |

**Frontend** — a single `public/index.html` file using:

- **Alpine.js** for reactive state — the whole wizard is one `x-data`
